...
- User-management: Erstellen / Speichern / Löschen + Ja confirm route through `command(UserManagement.{CreateUser,UpdateUserById,DeleteUserById})`.
- Per-tenant user-management features.spec, same shape.
- Duplicated `authenticateAdmin` / `createTestUser` helpers extracted into shared `UsersAdminPOM` + per-tenant subclasses; spec bodies use the fixture's `command` directly.

### 2b. Workflow POM (~9 sites)
- Invalid-weight `Packen` → `expectFailure: true`; "Wagen freigeben" / dialog-confirm "Ja" / "Stapeln" routed through `command`.
- Verpackt-tab cart release maps to `PackCarts.ReleaseCart` (not `Empty`), drops redundant `label` overrides where intl `_isLabel.true` already matched.

### 2c. Cross-workflow + pick + pack
- Cross-workflow block/unblock cart, via dynamic substring → later replaced with `CartManagement.ChangeBlockedState` vars shim.
- Pick-workflow "Alle Kommissioniert" position click awaits the `PickCarts.Full` toast.
- Pack-workflow release-cart "Ja" awaits `PackCarts.Release`.

### 2d. waitForResponse → toast wait where toast already proves the RPC
- 9 redundant `waitForResponse(Pack | Unpack | ConfirmPacking)` sites in `<workflow>/packing.spec.ts` collapsed into `command(rsc).click()`.

### 2e. Locator-override sweep (~22 sites, batches 1 & 2)
- Workflow POM picking/packing assigns; pack-workflow POM Cancel/MarkInStock/etc; cart-middleware dialog `<-` / `Nicht da` / `Doch da`; hold-carts.
- Other workflow POMs, shared/import, cross-workflow specs.

### 2f. Intl _isLabel sync
- `<workflow>/ShipList.UpdateBuildingBlockPallet` intl gains `_isLabel` so the rendered "Speichern" button is intl-derived; three POM sites drop the literal handleToast in favour of `command(...)`.

### 2g. Page-bound API + sweep
- `commandButton` fixture + `PomBase.commandButton` field; 14 POMs and 8 specs migrated to drop the explicit `page` argument; misc POMs promoted to extend `PomBase`.

### 2h. DX overhaul
- Rename `commandButton` → `command`; add `.via(locator)` and `.confirm()` fluent shortcuts (`.confirm()` later dropped in favour of `{ label: "Ja" }`).
- Drop unbound `Command.click(page, …)`; tighten resource id type to `ActionId`; missing intl entry now throws.
- Swap `BoundCommand.click` arg order so e2e options come first.
- `.thenConfirm()` replaces the recurring `afterClick: () => …getByRole("button", { name: "Ja" }).click()` pattern (7 sites).
- Drop `LocatorFactory` / `LocatorSource` types and the `{ locator: … }` option-bag; `.via(Locator)` is the only path to override the trigger. All 20 `(scope) => scope.X` sites migrated to `page.X` / `this.page.X`.
...
|---|---|---|
| `<workflow-scan>.ts:122` `scanItem` — `handleToast("Scannen und Drucken", () => this.scanInput.fill(gtin))` | Trigger is `Locator.fill()`, not `click()`. `command(...).click()` always issues `.click()` on the located element. | Keep. `scanInput.fill` is the natural API for the scan flow; wrapping `command` around it would require either a `trigger` option (one call site, not worth the helper surface) or a click-then-fill pattern that races with the input's debounce. Toast prefix is the literal intl value, no drift risk past a rename of `TruckItems.ScanAndPrint`. |

### 3b. `waitForResponse` direct (1 site)

| Location | Reason | Verdict |
|---|---|---|
| `<workflow>.ts:180` `packCartonOnRowExpectWeightCapError` — `waitForResponse(this.page, () => row.check(), PackListRsc.SaveItems)` | `SaveItems` runs with `Command.withDefaultToast({ onWaiting: null, onSuccess: null })` (the package Actions component) — silent on success, so there is no toast for `handleToast` to wait on. | Keep. This is the documented ranking-step-4 fallback. The same resource also appears as `waitForResponse: PackListRsc.SaveItems` in the `packAndWait` helper at `<workflow>.ts:380` — same justification. |

### 3b.1. `skipToast: true` on `command` (2 sites)

| Location | Reason | Verdict |
|---|---|---|
| `<workflow>.ts:191` `packCartonOnRowExpectWeightCapError` — `packArticlesBtn.click({ skipToast: true })` | Weight cap is enforced client-side: `Maximales Gewicht von 25 kg` is rendered as inline form-validation text, no `PackArticles` RPC fires. The neighbouring `expect(getByText(expectedError)).toBeVisible()` is the actual settle. | Keep. Initially migrated as `expectFailure: true`, which timed out waiting for a failure toast that never fires — corrected after running the suite. |
| `<workflow>.ts:159` `packCartonExpectInvalidThenRetry` (dead code path — no callers) — same shape | Same client-side weight-cap validation. | Keep for consistency with the live `…WeightCapError` sibling; will go away when this dead helper is removed. |

### 3b.2. Visible-state-change settle (rank #1 in `act.ts`) instead of toast (1 site)

| Location | Reason | Verdict |
|---|---|---|
| `<workflow>/flow.spec.ts:79` "Pick all items" — bare `positionButton.click()` followed by `expect(getByText("FREI")).toBeVisible({ timeout: 15000 })` | "Alle Kommissioniert" bursts N `PickList.Picked` toasts, then the watcher auto-fires `PickCarts.Full` which navigates via `Router.push` to `/carts`. The `Full` success toast races the navigation and does not reliably reach the new page. | Keep. Sweep first migrated this to `command(PickCarts.Full).via(...)` waiting on the Full toast — timed out under load. FREI tab visibility is a higher-rank settle anyway (rank #1) and was already in place. |
...
- Pack-workflow cancel/reverseCancel (`<workflow>.ts:65`, `:75`) — activator (`getByText("Abbrechen")` on the order card) and the "Ja" click are co-located, so the full chain reads `command(rsc).via(activator).thenConfirm().click()`.
...
| Location | Resource |
|---|---|
| `<workflow-a>.ts:219`, `:250`, `:389` + `<workflow-b>.ts:241` | `PackList.Print` (post-print "Ja" dialog; activator click in sibling POM method) |
| `<workflow>/packing.spec.ts:596`, `:610` | `<workflow>/PackList.ConfirmPacking` (activator triggered by `packCurrentOrderViaUI` helper) |
| `<workflow-scan>.ts:219` | `<workflow>/TruckItems.GodMode` (activator handled with a native `window.confirm` listener) |
...
|---|---|---|---|---|
| `<workflow>.ts:16` (`closeCartBtn` field) | `<workflow>/PickCarts.Full` | `"Wagen Abschließen"` (rendered on the main commission page) | `"Abschließen"` (the cart-completion *dialog* confirm — same flow, different button) | Same dialog-vs-activator split as above, just without a `_isLabel` ICU pattern. Sibling activator already uses `"Wagen Abschließen"`. |
| `<workflow>.ts:75` | `<workflow>/PickList.AddOrderToCart` | `"Auftrag hinzufügen"` (sidebar entry) | `"Hinzufügen"` (the per-order-row confirm button) | Same shape — main entry and per-row confirm need different copy. |
| `<workflow>/manager.spec.ts:255` | `<workflow>/Order.FixAddress` | plain `"Adressdaten überarbeiten"` | `"Speichern"` (the dialog submit) | The intl key has no `_isLabel` split; the dialog button uses generic `"Speichern"`. |
...
The workflow's `closeCartBtn` carries an inline comment justifying the label override; the other two are short enough that the override speaks for itself.
...
|---|---|---|
| **Non-button trigger** — `getByText`, `getByTestId`, list-row click, `<v-card>` clickable text | `<workflow-a>.ts:52` cart label, `<workflow-a>.ts:112` pack-spot text, `<workflow-b>.ts:113` detail-panel `Vergriffen markieren` span, `<workflow-scan>.ts:111` `dummy` printer text, `<workflow-c>.ts:107` `Wagen N` text | `getByRole("button", { name })` doesn't match `getByText` spans or `<v-list-item>` cards. Vue UI uses both for interactive surfaces. |
| **Regex / templated label** — button text contains a runtime value | `<workflow>/packing.spec.ts:289` `/Verpacken \(2\.5 kg\)/`, `:653` / `:703` `/Verpacken \(1 kg\)/` | `Command.label` is a string; weight is interpolated by the frontend into the button text. |
| **Scoped to dialog / row / parent** — many same-named buttons on page | `<workflow>/cart-middleware.spec.ts:56,73,91,131,140,248,259` `.v-dialog`-scoped, `<tenant>/carts-management.ts:120` `getByRole("dialog").getByRole("button", { name: "Wagen Blockieren" })`, `<workflow>.ts:444` `Kommissioniert` dialog opener (toast comes from the position click inside `afterClick`) | The default locator would match the wrong instance. `parent` covers some cases but not when the scoping is "the second button with this text" or "inside `.v-dialog` regardless of placement". |
| **Icon-only / accessible-name-less button** | `cross-workflow-unblock-visibility.spec.ts:69` the unblock icon — `cartRow.getByRole("button").filter({ hasNot: page.getByRole("link") }).first()` | Button has no text and no `aria-label`. The frontend renders it as an SVG; locating it requires a structural filter, not `getByRole({ name })`. |
...
|---|---|---|
| `CartManagement.ChangeBlockedState` | per-tenant `carts-management.ts` POMs, `cross-workflow-unblock-visibility.spec.ts` | Intl key exists in `action-intl.ts` but no Effect resource class — the action is dispatched via a frontend-only form. |
| `CartManagementForm.AdjustCartCount` | `cross-workflow-cart-count-sync.spec.ts:9` | Composite toast surfaced by the "Wagen verwalten" dialog; the dialog actually dispatches per-workflow `PickCarts.UpdateCount` mutations but the user-facing toast text comes from this synthetic intl key. |
...
1. **`Order.FixAddress` _isLabel** — adding `{_isLabel, true {Speichern} other {Adressdaten überarbeiten}}` to the FixAddress keys in `action-intl.ts` drops the manager.spec label override and unblocks similar use of FixAddress in other workflows. Confirm no on-page button currently relies on the long form being shown.
...
3. **`<workflow>/TruckItems.ScanAndPrint` trigger** — if a second fill-based toast case ever appears, extend `command` with an optional `trigger?: (loc: Locator) => Promise<void>` to absorb the last `handleToast` call site. Not worth it for a single instance.
