...
const mapMutation = <M extends Requests>(
  ...
  const mutations = Struct.keys(client).reduce(
    ...
    {} as {
      [Key in keyof typeof client as `${ToCamel<string & Key>}Mutation`]: (typeof client)[Key] extends
        RequestHandlerWithInput<
          infer I,
          infer A,
          infer E,
...
export const clientFor = <M extends Requests>(m: M) => useRuntime().runSync(clientFor_(m).pipe(Effect.map(mapClient)))
